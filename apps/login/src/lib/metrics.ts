...
const responseStatusCounter = meter.createCounter("http_server_response_status_total", {
...
const sessionCreationDuration = meter.createHistogram("login_session_creation_duration_seconds", {
 ...
 */
export function recordAuthFailure(method: string, reason: string, organization?: string): void {
  authFailures.add(1, {
    method,
    reason,
    organization: organization || "unknown",
 ...
 */
export function recordSessionCreationDuration(startTime: number, success: boolean): void {
  const duration = (Date.now() - startTime) / 1000;
